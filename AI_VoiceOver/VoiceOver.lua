...
function Addon:OnInitialize()
    ...
    if select(5, GetAddOnInfo("VoiceOver")) ~= "MISSING" then
        ...
        if not self.db.profile.SeenDuplicateDialog then
            ...
            {
                text = [[VoiceOver|n|n为了修复任务自动接受错误，我们必须重命名插件文件夹。 如果您看到此弹出窗口，则意味着旧的未自动删除。|n|n您可以安全地从 Addons 文件夹中删除“VoiceOver”。 “AI_VoiceOver”是新文件夹。]],                button1 = OKAY,
                timeout = 0,
                whileDead = 1,
                OnAccept = function()
    ...
    if select(5, GetAddOnInfo("AI_VoiceOver_112")) ~= "MISSING" then
        ...
        if not self.db.profile.SeenDuplicateDialog112 then
            ...
            {
                text = [[VoiceOver|n|nVoiceOver 1.12 端口已与其他版本合并在一起，不再作为单独的插件分发。|n|n您可以安全地从 Addons 文件夹中删除“AI_VoiceOver_112”。 “AI_VoiceOver”是新文件夹。]],                button1 = OKAY,
                timeout = 0,
                whileDead = 1,
                OnAccept = function()
    ...
    if not DataModules:HasRegisteredModules() then
        ...
        {
            text = [[VoiceOver|n|n未找到声音包。|n|n使用“/vo options”命令（或较新客户端中的界面选项）并转到“DataModules”选项卡以获取有关在何处下载声音包的信息。]],            button1 = OKAY,
            timeout = 0,
            whileDead = 1,
        }
